...
class ResultsView(ctk.CTkFrame):
    def __init__(self, master, result):
        ...
        for step_idx, step in enumerate(result.get("steps", [])):
            ...
            header_row_frame = ctk.CTkFrame(table_frame, fg_color="transparent")
            header_row_frame.pack(fill="x", pady=(0, 5))
            ...
            for display_row, real_row in enumerate(row_order):
                ...
                if isinstance(obj_value, Fraction):
                    if obj_value.denominator == 1:
                        obj_value_str = str(obj_value.numerator)
                    else:
                        obj_value_str = f"{obj_value.numerator}/{obj_value.denominator}"
                else:
