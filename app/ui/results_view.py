...
class ResultsView(ctk.CTkFrame):
    def __init__(self, master, result):
        ...
        for step_idx, step in enumerate(result.get("steps", [])):
            ...
            # ---------------- PINTAR LAS FILAS DE LA TABLA (MÁXIMO 4 FILAS) ----------------
            # Del backend vienen 5 filas. La fila 0 es la función objetivo R (Fase I).
            # Las filas 1, 2, 3 son las restricciones de las variables básicas.
            # Ignoramos por completo la fila 4 de la matriz.
            row_order = [0, 1, 2, 3]
            ...
            for display_row, real_row in enumerate(row_order):
                ...
                if real_row == 0:
                    if is_two_phase:
                        base_label = step.get("objective", "R")
                    else:
                        base_label = "Z"
                elif (real_row - 1) < len(basis_list):
                    raw_label = str(basis_list[real_row - 1]).lower().strip()
                    if raw_label == "rhs":
                        base_label = "SOL"
                    elif raw_label.startswith("a"):
                        base_label = raw_label.replace("a", "R").upper()
